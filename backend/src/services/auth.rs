...
pub mod middleware {
    ...
    /// Application state that includes the auth service.
    /// This trait allows the middleware to work with any state that contains AuthService.
    pub trait HasAuthService {
        fn auth_service(&self) -> &AuthService;
    }

    impl<S> FromRequestParts<S> for AuthUser
    where
        S: Send + Sync + HasAuthService,
    {
