...
export const useContentStore = defineStore("content", () => {
    ...
    async function fetchContents(
        ...
        try {
            ...
            error.value =
                e instanceof Error ? e.message : "Failed to fetch content list";
            throw e;
        } finally {
            loading.value = false;
        }
    ...
    async function searchContents(
        ...
        try {
            ...
            // **Implements: Requirement 6.1** - Set error state on failure
            error.value =
                e instanceof Error ? e.message : "Search content failed";
            return [];
        } finally {
            loading.value = false;
        }
    ...
    async function selectContent(content: ContentResponse): Promise<void> {
        ...
        try {
            ...
            const chapterList = await getContentApi(getToken).listChapters(
                content.id
            );
            ...
            error.value =
                e instanceof Error
                    ? e.message
                    : "Failed to retrieve the chapter list";
            throw e;
        } finally {
            loading.value = false;
        }
