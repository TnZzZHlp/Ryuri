...
export const useScanTaskStore = defineStore("scanTask", () => {
    ...
    const processingTasks = ref<ScanTask[]>([]);
    ...
    const taskByLibraryId = computed(() => {
        return (libraryId: number): ScanTask | undefined => {
            ...
            return (
                ...
                historyTasks.value.find(
                    (task) => task.library_id === libraryId
                ) ||
                processingTasks.value.find(
                    (task) => task.library_id === libraryId
                )
            );
        };
    ...
    async function fetchTasks(
        ...
        try {
            ...
            const response = await client.get<ListTasksResponse>(
                "/api/scan-tasks",
                {
            ...
            processingTasks.value = response.processing;
            historyTasks.value = response.history;
        } catch (e) {
            error.value =
                e instanceof Error ? e.message : "Failed to fetch scan tasks";
            throw e;
        } finally {
            loading.value = false;
        }
    ...
    async function fetchTask(taskId: string): Promise<ScanTask> {
        try {
            const client = getApiClient(getToken);
            const task = await client.get<ScanTask>(
                `/api/scan-tasks/${taskId}`
            );
            ...
            error.value =
                e instanceof Error ? e.message : "Failed to fetch scan task";
            throw e;
        }
    }
    ...
    async function triggerScan(libraryId: number): Promise<SubmitScanResponse> {
        try {
            ...
            const existingIndex = pendingTasks.value.findIndex(
                (t) => t.id === response.task.id
            );
            if (existingIndex === -1) {
            ...
            error.value =
                e instanceof Error ? e.message : "Failed to trigger scan";
            throw e;
        }
    }
    ...
    async function cancelTask(taskId: string): Promise<ScanTask> {
        try {
            const client = getApiClient(getToken);
            const task = await client.delete<ScanTask>(
                `/api/scan-tasks/${taskId}`
            );
            ...
            error.value =
                e instanceof Error ? e.message : "Failed to cancel scan task";
            throw e;
        }
    }
    ...
    function updateTaskInState(task: ScanTask): void {
        ...
        const pendingIndex = pendingTasks.value.findIndex(
            (t) => t.id === task.id
        );
        if (pendingIndex !== -1) {
        ...
        const historyIndex = historyTasks.value.findIndex(
            (t) => t.id === task.id
        );
        if (historyIndex !== -1) {
